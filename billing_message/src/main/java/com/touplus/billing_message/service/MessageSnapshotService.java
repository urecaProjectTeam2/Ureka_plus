...
// 이것도 배치 때문에 아예 내용 바뀔 것 같아서 아예 새로 만듦
package com.touplus.billing_message.service;
...
public class MessageSnapshotService {
    ...
    public int createSnapshotsBatch(List<Message> messages, MessageType messageType) {
        ...
        Map<Long, BillingSnapshot> billingMap =
                billingSnapshotRepository.findAllById(billingIds)
                        .stream()
        ...
        MessageTemplate template =
                messageTemplateRepository.findByMessageType(messageType)
                        .orElseThrow(() ->
                                new IllegalStateException("MessageTemplate not found: " + messageType));

        // 중복 생성된 스냅샷 제외
        Set<Long> existingMessageIds =
                messageSnapshotRepository.findExistingMessageIds(messageIds);
